...
local mime = require "sparrow.mime"
local enc = require "sparrow.encoding"
local sparrow = require "sparrow"
local httpd = require "sparrow.httpd"

sparrow.mimetypes = sparrow.mimetypes or {}
sparrow.encodings = sparrow.encodings or {}

-- gets the mimetype from the filename's extension
local function mimefrompath (path)
        local _,_,exten = string.find (path, "%.([^.]*)$")
        if exten then
                return sparrow.mimetypes [exten]
        else
                return nil
        end
end
...
local function encodingfrompath (path)
        local _,_,exten = string.find (path, "%.([^.]*)$")
        if exten then
                return sparrow.encodings [exten]
        else
                return nil
        end
end
